package com.retrobot.utils.automation;
...
public class NativeWindowsEvents {
    ...
    public static void rightClic(double x, double y) {
        int roundedX = (int) Math.round(x);
        int roundedY = (int) Math.round(y);
        log.debug("Clic {}, {}", roundedX, roundedY);
        long param = roundedX + (roundedY << 16);
        ...
        user32.SendMessage(hwnd, WM_RBUTTONDOWN, wparam, lparam);
        user32.SendMessage(hwnd, WM_MOUSEMOVE, wparam, lparam);
        user32.SendMessage(hwnd, WM_RBUTTONUP, wparam, lparam);
    }
